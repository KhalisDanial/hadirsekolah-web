...
async function fetchMuridAttendance() {
    ...
    const processedData = (data || []).map(att => {
        // FIX: Use att.timestamp since that is the column name in your Supabase table
        const scanDate = safeParseDate(att.timestamp || att.created_at);
...
window.openStudentCard = async (studentId) => {
    ...
    try {
        // --- 1. DAPATKAN SEMUA TARIKH IMBASAN (TEKNIK PAGINATION) ---
        let allDates = [];
        let fetchMore = true;
        let fromIndex = 0;
        const step = 1000;

        // Loop ini akan berputar menarik data 1000 demi 1000 sehingga habis
        while (fetchMore) {
            ...
            if (data && data.length > 0) {
                ...
                if (data.length < step) {
                    fetchMore = false; // Berhenti jika baki data kurang dari 1000
                } else {
                    fromIndex += step; // Teruskan ke 1000 seterusnya
                }
            } else {
                fetchMore = false;
            }
        ...
        let officialSchoolDates = new Set(); 
...
window.openStaffCard = async (staffId) => {
    ...
    try {
        // --- 1. DAPATKAN SEMUA TARIKH IMBASAN KESELURUHAN SEKOLAH (PAGINATION) ---
        let allDates = [];
        let fetchMore = true;
        let fromIndex = 0;
        const step = 1000;
